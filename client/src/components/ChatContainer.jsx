...
const ChatContainer = () => {
    ...
    const handleSendImage = async (e) => {
        ...
        reader.onloadend = async () => {
            await sendMessage({ image: reader.result });// send image to server
            e.target.value = ""
        }
    ...
    return selectedUser ? (
        <div className='h-full overflow-y-auto relative backdrop-blur-lg'>
                ...
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                        {msg.image ? (
                            ...
                            <p className={`p-2 max-w-[200px] md:text-sm font-light rounded-lg mb-8 break-all bg-[#3a99e1] text-white ${msg.senderId === authUser._id ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
                        )}
                        <div className='text-center text-xs'>
                            ...
                            <p className='text-gray-900 pt-0.5'>{formatMessageTime(msg.createdAt)}</p>
                        </div>
                    </div>
                ))}
                <div ref={scrollEnd}>
            ...
            <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
                ...
                <img onClick={handleSendMessage} src={assets.send_button1} alt="" className='w-10 h-10 cursor-pointer' />
            </div>
        </div>
    ) : (
        <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden'>
            <img src={assets.logo} alt="" className='max-w-16' />
            <p className='text-lg font-medium text-white'>Your people, just a ping away!</p>
        </div>
    )
}
